...
fn is_browser(app_name: &str) -> bool {
    let browsers = ["Safari", "Chrome", "Google Chrome", "Firefox", "Mozilla Firefox", 
                   "Brave", "Brave Browser", "Arc", "Edge", "Microsoft Edge"];
...
fn get_browser_tab_via_applescript(app_name: &str) -> Result<String, MacOSError> {
    let script = if app_name.contains("Chrome") || app_name.contains("Google Chrome") {
        ...
        return Err(MacOSError::CoreGraphicsError("Firefox doesn't support tab AppleScript".to_string()));
    } else {
    ...
    Err(MacOSError::CoreGraphicsError("No tab title found".to_string()))
}